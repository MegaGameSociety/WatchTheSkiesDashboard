class PublicRelation < ActiveRecord::Base
  ...
  def self.country_status(country, game)
    game.public_relations.where(country: country).group(:round).group(:source).sum(:pr_amount).sort_by{|key,value| key[0]}.reverse!
  end

  # Get all PR amounts for a given round
  def self.round_pr(round, game)
    game.public_relations.where(round: round).group(:country).group(:source).sum(:pr_amount)
  end
end
