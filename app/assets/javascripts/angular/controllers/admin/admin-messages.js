(function() {
  ...
  angular.module('wtsApp').controller('adminMessageCtrl', ['$scope', '$http', '$interval', function($scope, $http, $interval) {
    $scope.teamId = 1;
    $scope.messages = [];
    ...
    $scope.filterFn = function(conversation) {
      ...
      if (filter === 0) {
        ...
        return conversation.latest_message.sender_id !== $scope.teamId;
      } else if (filter === 2) {
        return conversation.latest_message.sender_id === $scope.teamId;
      } else {
        return true;
    ...
    var groupMessages = function(messages) {
      ...
      .groupBy(function(message) {
        ...
        return message.sender_id === teamId ? message.recipient_id : message.sender_id;
      })
      .map(function(messages) {
        // Get the Team name that the conversation is with.
        var conversation_partner_id = messages[0].sender_id === teamId ? messages[0].recipient_id : messages[0].sender_id;
    ...
    $scope.changeTeam = function() {
      $http.get('/api/admin/messages/' + $scope.teamId).then(
        function successCallback(response) {
          ...
          $scope.messages = pushNewMessages(_.sortBy(newMessages, 'updated_at'));
        },
        function errorCallback(response) {
